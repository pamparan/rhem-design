...
export const mockDevices: Device[] = [
  ...
  ...Array.from({ length: 35 }, (_, i) => ({
    ...
    status: i % 8 === 0 ? 'SUSPENDED' as const :
           i % 7 === 0 ? 'PENDING_SYNC' as const :
           i % 6 === 0 ? 'ERROR' as const :
           i % 5 === 0 ? 'DEGRADED' as const :
           i % 4 === 0 ? 'REBOOTING' as const :
           i % 3 === 0 ? 'OFFLINE' as const : 'ONLINE' as const,
    applicationStatus: i % 4 === 0 ? 'ERROR' as const :
                      i % 3 === 0 ? 'DEGRADED' as const :
                      i % 2 === 0 ? 'UNKNOWN' as const : 'HEALTHY' as const,
    systemUpdateStatus: i % 5 === 0 ? 'FAILED' as const :
                       i % 4 === 0 ? 'OUT_OF_DATE' as const :
                       i % 3 === 0 ? 'UPDATING' as const :
                       i % 2 === 0 ? 'ROLLING_BACK' as const : 'UP_TO_DATE' as const,
    type: ['Gateway', 'Sensor', 'Compute', 'Router', 'Storage'][i % 5],
    location: ['New York', 'San Francisco', 'Los Angeles', 'Chicago', 'Miami', 'Boston', 'Seattle', 'Denver'][i % 8],
    ip: `192.168.1.${40 + i}`,
    firmware: `v${Math.floor(i / 10) + 1}.${i % 10}.${Math.floor(Math.random() * 10)}`,
    fleet: i % 3 === 0 ? undefined : 'Fitting Room Devices',
    lastSeen: `${Math.floor(Math.random() * 7) + 1} days ago`,
    configVersion: i % 8 === 0 ? 120 + Math.floor(Math.random() * 20) : undefined
  }))
];

export const mockSystemState: SystemState = {
  isPostRestore: true,
  suspendedDeviceCount: mockDevices.filter(d => d.status === 'SUSPENDED').length,
  pendingSyncDeviceCount: mockDevices.filter(d => d.status === 'PENDING_SYNC').length,
